/**
 * @dsh-xhl/dsh-plugin-terminal — host half.
 *
 ...
 */
/** Remove Volta's injected tool-image directories from a PATH string.
 *  `volta run` (which launches the DSH host when installed as a Volta package)
 *  prepends the resolved tool image dir — e.g.
 *    C:\Users\...\Volta\tools\image\node\24.13.0   (Windows)
 *    ~/.volta/tools/image/node/24.13.0/bin          (Unix)
 *  — to PATH so the host itself runs the pinned Node. If a PTY child inherits
 *  that PATH unchanged, `node`/`npm`/`pnpm` inside the terminal resolve to the
 *  host's pinned binary directly and Volta's per-project switching (package.json
 *  "volta", .node-version) is bypassed. Strip every segment under
 *  <volta>/tools/image/ so the child resolves tools through Volta's shims,
 *  exactly like a freshly opened system terminal. */
function stripVoltaImageDirs(pathValue) {
  if (typeof pathValue !== 'string' || pathValue.length === 0) return pathValue
  return pathValue
    .split(delimiter)
    .filter((seg) => !/volta[\\/]tools[\\/]image[\\/]/i.test(seg))
    .join(delimiter)
}
...
export function buildSessionEnv(base = process.env) {
  ...
  // Strip Volta's injected tool-image dirs from PATH. Windows stores this as
  // "Path" while Unix uses "PATH", and spreading process.env keeps the original
  // case — find the real key so we edit in place and never leave a duplicate
  // (case-insensitive) PATH entry in the child env block.
  const pathKey = Object.keys(env).find((k) => k.toLowerCase() === 'path')
  if (pathKey !== undefined && typeof env[pathKey] === 'string') {
    env[pathKey] = stripVoltaImageDirs(env[pathKey])
  }
  // The host runs under `volta run`, which injects internal state that must not
  // leak into a user's terminal: _VOLTA_TOOL_RECURSION makes Volta's node/npm
  // shims "delegate to system" (skipping per-project resolution), and NODE_PATH
  // points at Volta's shared module dir. Clearing them — together with the image
  // dir above — is what actually restores package.json "volta" switching.
  delete env._VOLTA_TOOL_RECURSION
  delete env.VOLTA_TOOL_RECURSION
...
/** Settings namespace (lowercase kebab identifier).
 *  Newer @deepseek-ai/dsh-settings dropped the settingsNamespace() branding
 *  helper; the service now accepts/validates a plain lowercase-kebab string
 *  directly via SettingsProvider.register(). */
...
/** Per-workspace spawn counter (keyed by resolved cwd): the `#N` in a
 *  session title labels the Nth terminal spawned in THAT workspace, so tabs in
 *  different workspaces don't share one global sequence (workspace A's first
 *  tab reads "A 1", not "A 7" just because other workspaces used up the
 *  counter). Resets with the process, like the id counter above. */
const workspaceCounts = new Map()

export const name = '@dsh-xhl/dsh-plugin-terminal'
...
export function apply(ctx) {
   ...
   *  resolver degrades to the process cwd in that case. Resolved lazily at
   *  request time - cordis `ctx.get` only sees services already started, and
   *  dsh-workspace may start AFTER this plugin, so a top-level snapshot would
   *  always be undefined. */
  const getWorkspaceRegistry = () => (typeof ctx.get === 'function' ? ctx.get('workspaceRegistry') : undefined)
  ...
  function createSession({ cols = 80, rows = 24, cwd, shell, cmdline, seed = '', sessionId } = {}) {
    ...
    const id = makeId()
    const sessionCwd = resolveSessionCwd(cwd, sessionId, getWorkspaceRegistry())
    ...
    /** session record */
    const workspaceKey = sessionCwd || '__default__'
    const workspaceNo = (workspaceCounts.get(workspaceKey) ?? 0) + 1
    workspaceCounts.set(workspaceKey, workspaceNo)
    const record = {
      id,
      pty,
      shell: file,
  ...
  const disposeRoute = webServer.register({
    ...
    async handler(req, res) {
      ...
      try {
        ...
        // GET /workspaces — list every DSH workspace (id + path + display title)
        // so the client can render a per-workspace auto-open settings panel.
