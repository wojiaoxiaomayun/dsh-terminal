 ...
 *
 * Layout: dual registration, mutually exclusive by conversation phase. The
 * session copy mounts in conversation.composer.dock (in flow below the
 * composer card and the chat stats line); the hero copy mounts in
 * conversation.input.dock wrapped in HeroGate. composer.dock is
 * composer-variant only and never renders on the welcome screen, while
 * input.dock renders in BOTH phases - so HeroGate reads the
 * ConversationRoot ancestor's data-phase attribute (MutationObserver -
 * hero can carry a sessionId, so sessionId is NOT a hero signal) and keeps
 * the hero copy unmounted outside the hero phase. Exactly one live panel
 * per phase; two would double the boot restore and the toggle keydown
 * listener. The InputBar root pads its flow by
 * `--dsh-composer-side-clearance` (16px each side), which would leave the
 * session copy 32px narrower than the column - `.dshTermRoot` breaks out
 * with negative side margins to span the full column again, and carries a
 * 10px margin-top so it does not sit flush against the stats line above;
 * the -8px margin-bottom cancels the InputBar root's own 8px bottom padding
 * so the panel rests flush against the column bottom (composerSeat is
 * pinned with bottom:0, so that padding was the only gap left below). The
 * hero copy is absolutely positioned over the column via the
 * `[data-phase=hero]` rule (its containing block is composerSeat); that rule
 * also resets margin AND width - left:0/right:0 with the session copy's
 * breakout width would be over-constrained (LTR drops `right`), shifting
 * the root 32px right and pushing the header's right-edge controls
 * (settings, collapse) out of view.
 * Collapsed it is a single 34px bar; expanded it grows with a drag-to-resize
 * grip. A configurable shortcut (default Ctrl+`) toggles it - see
...
function TerminalPanel(props) {
  ...
  /** panel root element (used to locate the composer seat for stacking) */
  ...
  /* The terminal sits inside the composer seat (z-index 7), which paints BELOW
   * the column width-resize handles (z-index 8). Since the terminal now spans
   * the full column, those handles would steal pointer events over its gutter
   * edges. Raise the seat to 9 (DSH's own :has([data-trigger-menu]) bump) so the
   * terminal — and the composer it lives in — sit above the handles. */
  const { useLayoutEffect } = React;
  useLayoutEffect(() => {
    const rootEl = rootRef.current;
    if (rootEl === null) return;
    const seat = rootEl.closest("[data-composer-seat]");
    if (seat === null) return;
    seat.style.zIndex = "9";
    return () => { seat.style.zIndex = ""; };
  }, []);
  ...
  return React.createElement(
    "div",
    { className: "dshTermRoot", ref: rootRef },
    open
      ? React.createElement(
          "div",
...
/* Hero-phase gate for the input.dock fallback copy. Renders the panel only
 * while the owning conversation root is in the hero (welcome) phase. The
 * root marks its phase via data-phase ("hero" | "active" | "settling");
 * hero can carry a sessionId (blank session), so the attribute - not the
 * session id - is the hero signal. A MutationObserver tracks hero <-> active
 * transitions without remounting the conversation, so the hero copy mounts
 * and unmounts cleanly at each edge (its boot restore and keydown listener
 * come and go with it) and never coexists with the composer.dock copy. The
 * gate host div stays mounted (display:contents) to keep the observer alive
 * while rendering nothing outside the hero phase. */
function HeroGate({ children }) {
  ...
  useEffect(() => {
    ...
    if (el === null) return;
    const root = el.closest("[data-phase]");
    if (root === null) return;
    const read = () => setIsHero(root.getAttribute("data-phase") === "hero");
    read();
  ...
  return React.createElement("div", { ref, className: "dshTermHeroGate" }, isHero ? children : null);
}

/* input.dock entry: the same panel, gated to the hero phase. Props flow
 * through unchanged - the input.dock owner face is what the panel used
 * before the composer.dock move, so no prop mapping is needed. */
function TerminalPanelHero(props) {
  return React.createElement(HeroGate, null, React.createElement(TerminalPanel, props));
}
export { TerminalPanel, TerminalPanelHero };
