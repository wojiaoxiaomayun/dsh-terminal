 ...
 *
 * Layout: injected into conversation.input.dock, in-flow in the composer
 * stack - the only dock slot that also renders on the new-session hero screen
 * (composer.dock is composer-variant only). The stack places this slot above
 * the input box in DOM order; `.dshTermRoot{order:2}` sinks it visually below
 * the input card and its stats/usage line. Hero (welcome) centers the whole
 * composer vertically, so there `[data-phase=hero]` switches the panel to
 * absolute bottom-of-column instead - nothing to cover there, no chat exists.
