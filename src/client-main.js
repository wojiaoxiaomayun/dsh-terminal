 ...
 *
 * Layout: one registration in conversation.input.dock, the conversation
 * contract's "full-width entries above the composer card". That slot renders
 * in BOTH phases, so a single panel covers the welcome screen and a live
 * session alike - no second copy, and therefore no double boot restore or
 * duplicate toggle keydown listener. `.dshTermRoot{order:2}` sinks the panel
 * below the composer bar in the stack's visual order, so it lands under the
 * input card and its context meter at the column bottom. hero centers a
 * narrower composer column, so there the `[data-phase=hero]` rule switches
 * the panel to absolute bottom-of-column instead - nothing to cover there,
 * no chat exists.
 *
 * The earlier composer.dock placement is NOT usable for a full-width panel:
 * that slot renders inside the InputBar's `.dock` row, a centered
 * content-hugging flex line shared with the context meter, so the panel
 * collapses to one flex item beside the meter (~640px, off-center) instead
